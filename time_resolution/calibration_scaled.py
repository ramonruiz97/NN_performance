...
def linear_fit(sigmat, sigmaeff, sigmaeff_err, sigmat_err, av):
  ...
  _pars.add(dict(name='p0p', value=0, min = 0., max=500., free=True))
...
def calibration(data=False, 
  ...
  if figs:
    ...
    label = rf"$B_d^0 \rightarrow J/\psi K^*$"
    if "DsPi" in mode:
      label = rf"$B_s^0 \rightarrow D_s^- \pi^+$"
...
if __name__ == '__main__':
  ...
  parser.add_argument('--json-mcsig', help='Signal MC of each bin')
  parser.add_argument('--json-mcprompt', help='MC prompt of each bin')
  ...
  p_mcsig = [ipanema.Parameters.load(p) for p in args['json_mcsig'].split(',')]
  p_mcprompt = [ipanema.Parameters.load(p) for p in args['json_mcprompt'].split(',')]

  for i in range(len(p)):
    _ = ufloat(p[i]["sigmaeff"].value, p[i]["sigmaeff"].stdev)
    mcprompt = ufloat(p_mcprompt[i]["sigmaeff"].value, p_mcprompt[i]["sigmaeff"].stdev)
    mcsig = ufloat(p_mcsig[i]["sigmaeff"].value, p_mcsig[i]["sigmaeff"].stdev)
    sigmaeff = mcsig/mcprompt*_
    print(f"Ratio {mcsig/mcprompt}")
    print(sigmaeff)
    p[i]["sigmaeff"].set(value=sigmaeff.n, stdev=sigmaeff.s)
  ...
  if "Bs" in mode:
    pars["p0"].set(value=pars["p0"].value/1000, stdev=pars["p0"].stdev/1000)
    pars["p0p"].set(value=pars["p0p"].value/1000, stdev=pars["p0p"].stdev/1000)
    pars["average"].set(value=pars["average"].value/1000, stdev=pars["average"].stdev/1000)
  print(pars)
