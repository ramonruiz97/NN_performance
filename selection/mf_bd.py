...
def ipatia_exponential(mass, signal, fsigBd=0, fsigBs=0, fexp=0,
    muBd=0, muBs=0, sigmaBd=10, sigmaBs=11, sigmafrac=1.1155, lambd=0, zeta=0, beta=0,
...
def mass_fitter(
    ...
    pars.add(
             dict(name="mLL", value=mLL, free=False)
    )
    pars.add(
             dict(name="mUL", value=mUL, free=False)
    )
    pars.add(
             dict(name=f"fsigBd", value=0.9, min=0.2, max=1, free=True, latex=r"N_{B_d}")
          )
    pars.add(dict(name="muBd", value=5280, min=5150, max=5450, latex=r"\mu_{B_d}"))
    pars.add(
            dict(
                name="sigmaBd",
                value=11,
    ...
    pars.add(dict(name="fsigBs", value = 0.05, min=0, max=0.1, free=True, latex=r"f_{Bs}"))
    pars.add(dict(name="muBs", formula=f"muBd+87.19", latex=r"\mu_{B_s^0}"))
    pars.add(dict(name="sigmafrac", value=0.984, min=0., max=2., free=False,
                latex=r"\sigma_{B_d^0}/\sigma_{B_s^0}"))
    pars.add(dict(name="sigmaBs", formula="sigmaBd/sigmafrac",
                latex=r"\sigma_{B_s^0}"))

    if input_pars:
      _pars = ipanema.Parameters.clone(input_pars)
      _pars.lock()
      _pars.remove("fsigBd", "muBd", "sigmaBd", "fsigBs", "muBs", "sigmaBs")
      _pars.unlock("b")
      pars = pars + _pars
    else:
      #Prefit stage
      pars["fsigBd"].value = 1
      pars["fsigBd"].free = False
      ...
      pars.add(dict(name='fexp', formula="1-fsigBd-fsigBs", latex=r'f_{comb}'))
    ...
    if "fsigBd" in _p:
        ...
        _p["fexp"].set(value=1-fpars["fsigBd"].value, min=-np.inf, max=np.inf)
    ...
    if "fexp" in _p:
        ...
        _p["fsigBd"].set(value=fpars["fsigBd"].value, min=-np.inf, max=np.inf)
        _p["fsigBs"].set(value=0, min=-np.inf, max=np.inf)
    ...
    if "fsigBs" in _p:
        _p["fexp"].set(value=0, min=-np.inf, max=np.inf)
        _p["fsigBd"].set(value=0, min=-np.inf, max=np.inf)
        _p["fsigBs"].set(value=fpars["fsigBs"].value, min=-np.inf, max=np.inf)
    #Plot fBs
    _x, _y = ristra.get(mass), ristra.get(
        pdf(mass, signal, **_p.valuesdict(), norm=hdata.norm)
    )
    # axplot.plot(_x, _y, color="C3", label=rf"$B^+$ {model}")
    axplot.plot(_x, _y, color="C4", label=rf"$B^0_s$")
...
if __name__ == "__main__":
    ...
    if sweights:
      if "root" in args["sweights"]:
        ...
        for w in ["fsigBd", "fexp","fsigBs"]: #TODO: fix
          _weight = sw
          _proxy += _weight[w]
