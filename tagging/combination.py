...
def cal(id, eta, p0=0, p1=0, p2=0, dp0=0, dp1=0, dp2=0, eta_bar=0):
  id /= abs(id)
  result = 0
  result += (p0 + id * 0.5 * dp0)
  result += (p1 + id * 0.5 * dp1) * (eta - eta_bar)
  result += (p2 + id * 0.5 * dp2) * (eta - eta_bar) * (eta - eta_bar)
  return result
  ...
  if final_comb:
    ...
    if ss_calibrations:
        ...
        for i, eta_b in enumerate(etass):
          p = ipanema.Parameters.load(ss_calibrations[i])
          omega.append(cal(data.df[f"{idb}"], data.df[f"{eta_b}"], **p.valuesdict()))
