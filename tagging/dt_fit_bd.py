...
def fcn(pars, data):
    ...
    p = pars.valuesdict()
    _p = ipanema.Parameters.clone(pars)
     
    c_b = ipanema.Parameters.build(pars, pars.find("c.*b"))
    c_u = ipanema.Parameters.build(pars, pars.find("c.*u"))
    
    for k in (c_b + c_u).keys():
      _p.remove(k)

    pars_b = _p + c_b
    pars_u = _p + c_u
    plist = []
    plist.append(pars_b.valuesdict())
    plist.append(pars_u.valuesdict())
    
    chi2 = []
    trigger = ["biased", "unbiased"]

    for i, t  in enumerate(trigger):
      prob = model_spline(data[t].x, data[t].prob, **plist[i])
      chi2.append(-2.*np.log(prob)*ipanema.ristra.get(data[t].weight))
    ...
    chi2_gauss  /= len(chi2conc)
    return chi2conc + chi2_gauss
...
def calibration_ss(tp, time_pars, mode, tagger, DP=False,
  ...
  trigger = ["biased", "unbiased"]
  vars = ["q", "eta", "id", "time", "sigmat"]
  ddata = {}
  den_etab = 0
  num_etab = 0
  data = ipanema.Sample.from_root(tp)
  data.df["q"] = data.df[q_b]
  data.df["eta"] = data.df[eta_b]
  b_id = 'B_TRUEID' if 'MC' in mode else 'B_ID'
  data.df["id"] = data.df[b_id]
  data.chop(cut)
  corr = np.sum(data.df.eval(weight))/np.sum(data.df.eval(f"{weight}*{weight}"))
  data.df["weight"] = data.df.eval(f"{weight}*{corr}")
  for t in trigger:
    tc = trigger_cut(t, cut)
    tc = tc.replace("eta", eta_b)
    tc = tc.replace("q", q_b)
    ddata[t] = ipanema.Sample.from_root(tp, cuts=tc)
    ddata[t].df["q"] = ddata[t].df[q_b]
    ddata[t].df["eta"] = ddata[t].df[eta_b]
    ddata[t].df['id'] = ddata[t].df[f'{b_id}']
    # print(weight)
    # print(ddata[t].df.eval(weight))
    corr = np.sum(ddata[t].df.eval(weight))/np.sum(ddata[t].df.eval(f"{weight}*{weight}"))
    ddata[t].df["weight"] = ddata[t].df.eval(f"{weight}*{corr}")
    den_etab += ddata[t].df["weight"].sum()
    num_etab += ddata[t].df.eval("weight*eta").sum()
    ddata[t].allocate(x=vars, weight="weight", prob="0*weight")

    print("Number of events: ", ddata[t].df.shape[0])
    print("Number of weighted events: ", np.sum(ddata[t].df.eval(f'{weight}')))
    print("Number of weighted events corrected: ", np.sum(ddata[t].df['weight']))
  ...
  pars.add(dict(name=f'c1u', value=0.300, min=0., max=100., free=True)) #Convention
  pars.add(dict(name=f'c2u', value=0.789, min=0., max=100., free=True))
  pars.add(dict(name=f'c3u', value=0.630, min=0., max=100., free=True))
  pars.add(dict(name=f'c4u', value=1.2, min=0., max=100., free=True))
  pars.add(dict(name=f'c5u', value=1.2, min=0., max=100., free=True))
  pars.add(dict(name=f'c6u', value=1.7, min=0., max=100., free=True))
  pars.add(dict(name=f'c7u', value=1., min=0., max=100., free=False))
  pars.add(dict(name=f'c8u', formula = f'c7u + (c6u-c7u)*(15-12)/(3-12)'))

  pars.add(dict(name=f'c1b', value=0.300, min=0., max=100., free=True)) #Convention
  pars.add(dict(name=f'c2b', value=0.789, min=0., max=100., free=True))
  pars.add(dict(name=f'c3b', value=0.630, min=0., max=100., free=True))
  pars.add(dict(name=f'c4b', value=1.2, min=0., max=100., free=True))
  pars.add(dict(name=f'c5b', value=1.2, min=0., max=100., free=True))
  pars.add(dict(name=f'c6b', value=1.7, min=0., max=100., free=True))
  pars.add(dict(name=f'c7b', value=1., min=0., max=100., free=False))
  pars.add(dict(name=f'c8b', formula = f'c7b + (c6b-c7b)*(15-12)/(3-12)'))
  ...
  pars['eta_bar'].set(value=num_etab/den_etab)
    
  print("Parameters before the fit")
  print(pars)

  res = ipanema.optimize(fcn, params= pars, fcn_args=(ddata,),
  ...
  if figs:
    ...
    for eta_lo, eta_hi in binning:
      ...
      binweights = ddata["unbiased"].df.query(eta_cut)['weight']
      sumW = np.sum(binweights)
      eta.append(np.sum(ddata["unbiased"].df.query(eta_cut)['eta'].array * binweights) / sumW)
      df = ddata["unbiased"].df.query(eta_cut)
...
if __name__ == '__main__':
  ...
  pars = calibration_ss(tp, time_pars, mode, tagger,  DP, order, weight, figs=args["output_plots"])
