...
def dn_cal(eta, om, 
  ...
  const = w * (1.-2.*om)
  deriv = np.array([
                    np.sum(const*(0.5*(eta - 0.5) + q*0.5*(eta -0.5))),
                    np.sum(const*(0.5*(eta - 0.5) - q*0.5*(eta -0.5)))
                   ])
...
if __name__ == '__main__':
  ...
  for t in taggers:
    ...
    for j, b in enumerate(bins):
      ...
      norm = total_tag[w_b].sum()

       
      nwts = np.sum(total_tag[w_b])
      dsq =  dn(tot_om, total_tag[w_b].values, 2)
      dsq_stat_err = np.sqrt((dn(tot_om, total_tag[w_b].values, 4) - (dn(tot_om, total_tag[w_b].values, 2)**2))/(nwts-1))
      ...
      N = total[w_b].sum()
      Nt = total_tag[w_b].sum()
      Neff = (total[w_b].sum())**2/total.eval(f"{w_b}*{w_b}").sum()
